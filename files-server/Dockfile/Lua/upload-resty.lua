...
local _M.uploadHandle()
    local resposeData={code=500,msg='upload fail',desc='上传失败',server_verison= conf.version,file_orign_name= orign_name_array,diskstatus={diskuse=gcommon.getdiskuse(),diskspace= gcommon.getdiskspace()}}
    local file
    local file_name
    ...
    while true do
        ...
        if typ == "header" then
            if res[1] == "Content-Disposition" then
                filename = gcommon.getFilename(res[2])
                if filename then
                    local extension = gcommon.get_extension(filename)
                    ...
                    -- 如果文件扩展名命中-- 使用文件原来的名字
                    if gcommon.in_array(extension, conf.allow_exts) then
                        local file,err=io.open(dir..filename)
                        if file == nil then 
                            ngx.log(ngx.ERR,"ERR....filename: "..filename)
                            file_name = dir..gcommon.getFileRealName(filename)
                            else
                            -- 文件名重复处理 时间戳_文件名
                            file.close()
                            file_name = dir..os.time().."_"..gcommon.getFileRealName(filename)
                            end
                    else
                        local file_id = gcommon.uuid2()
        ...
        elseif typ == "eof" then
            if file_name then 
                ...
                ngx.say(cjson.encode({code=509, msg='form name do not existed',desc='',server_verison= conf.version,file_orign_name= orign_name_array,diskstatus= {diskuse= gcommon.getdiskuse(),diskspace= gcommon.getdiskspace()}}))
        end
            break
            else
            end
